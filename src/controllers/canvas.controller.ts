...
export const getCanvas = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    if (!room) {
      res.status(404).json({ errors: [{ status: '404', title: 'Room Not Found', detail: 'Room not found or inactive' }] });
      return;
    }

    if (!req.userId && !room.allowGuests) {
      res.status(403).json({ errors: [{ status: '403', title: 'Forbidden', detail: 'This room does not allow guests' }] });
      return;
    }

    const canvas = await getOrCreateCanvas(roomId);
    if (!canvas) {
      res.status(404).json({ errors: [{ status: '404', title: 'Room Not Found', detail: 'Room not found' }] });
      return;
    }
    ...
    res.status(500).json({ errors: [{ status: '500', title: 'Internal Server Error', detail: 'Failed to fetch canvas' }] });
  }
};
...
export const addCanvasOperation = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    if (!userId) {
      res.status(401).json({ errors: [{ status: '401', title: 'Unauthorized', detail: 'User must be authenticated to draw' }] });
      return;
    }
    ...
    if (!operationData.operations?.length) {
      res.status(400).json({ errors: [{ status: '400', title: 'Bad Request', detail: 'Invalid operation data' }] });
      return;
    }
    ...
    res.status(500).json({ errors: [{ status: '500', title: 'Internal Server Error', detail: 'Failed to add drawing operation' }] });
  }
};
...
export const deleteCanvasOperations = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    if (!userId) {
      res.status(401).json({ errors: [{ status: '401', title: 'Unauthorized', detail: 'User must be authenticated to delete operations' }] });
      return;
    }
    ...
    if (!Array.isArray(operationIds) || operationIds.length === 0) {
      res.status(400).json({ errors: [{ status: '400', title: 'Bad Request', detail: 'Operation IDs array is required' }] });
      return;
    }

    const canvas = await db.query.canvases.findFirst({ where: eq(canvases.roomId, roomId) });
    if (!canvas) {
      res.status(404).json({ errors: [{ status: '404', title: 'Canvas Not Found', detail: 'Canvas not found for this room' }] });
      return;
    }

    const stringIds = operationIds.filter((id): id is string => typeof id === 'string');
    const deleted = await db.delete(canvasOperations)
      .where(and(eq(canvasOperations.canvasId, canvas.id), inArray(canvasOperations.opId, stringIds)))
      .returning();

    logger.info(`[CANVAS] Deleted ${deleted.length} operations from room ${roomId} by user ${userId}`);

    const remaining = await db.select({ count: canvasOperations.id })
      .from(canvasOperations)
      .where(eq(canvasOperations.canvasId, canvas.id));

    res.status(200).json({ data: { deletedCount: deleted.length, remainingOperations: remaining.length } });
  } catch (error) {
    logger.error('Error deleting canvas operations:', error);
    res.status(500).json({ errors: [{ status: '500', title: 'Internal Server Error', detail: 'Failed to delete canvas operations' }] });
  }
};
